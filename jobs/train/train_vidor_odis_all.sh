...
###############################################################################
VIDOR_ZIP=/scratch/project_462000938/vidor/dataset.zip
...
NAME=vidor_odis_${loss_suffix_i}_probabilistic_alldataset 
...
srun --cpu-bind=mask_cpu=${CPU_MASKS} \
     ...
     "${CODE_DIR}/run.sh" \
       python -u "${CODE_DIR}/VIDOR_ODIS/ibot_vidor_obj.py" \
            --data_path "${TMP_VIDOR}/dataset/extracted_frames" \
            --boxes_root "${TMP_VIDOR}/dataset" \
