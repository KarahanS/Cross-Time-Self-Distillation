...
class iBOTHead(DINOHead):
    ...
    def project_cls_token(self, cls_token):
        """
        Project only the [CLS] token(s) to out_dim using the CLS branch.
        Accepts [N, E] or [..., E]; returns with the same leading dims and out_dim.
        Mirrors the logic used in iBOTHead.forward for CLS.
        """
        x = cls_token
        if self.last_layer is None:
            # bottleneck_dim == 0 (no weight-norm head)
            # forward() does: x = self.mlp[:-1](...) then self.mlp[-1] for CLS
            if (
                isinstance(self.mlp, (nn.Sequential, CustomSequential))
                and len(self.mlp) >= 1
            ):
                if len(self.mlp) > 1:
                    x = self.mlp[:-1](x)
            ...
            # bottleneck_dim > 0 (weight-norm head present)
            x = self.mlp(x)
            x = nn.functional.normalize(x, dim=-1, p=2)
            proj = self.last_layer(x)

        if self.last_norm is not None:
            proj = self.last_norm(proj)
        return proj
