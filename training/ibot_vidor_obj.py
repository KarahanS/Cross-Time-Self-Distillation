...
from loader_vidor_odis import DataAugmentationSingleObject, VidORiBOTFrameDatasetODIS
import wandb
import utils
...
def get_args_parser():
    ...
    parser.add_argument("--instance_level", default=False, type=utils_ibot.bool_flag)
    parser.add_argument(
        "--mode",
        choices=["predefined-clips", "probabilistic"],
        default="predefined-clips",
        type=str,
    )

    parser.add_argument(
        "--neighbour_time_window",
        default=(1.0, 4.0),
        type=float,
        nargs=2,
        help="Temporal window for neighboring frames (min, max)",
    )
    parser.add_argument(
        "--neighbour_tries",
        default=20,
        type=int,
        help="Number of tries to find valid neighboring frames",
    )
...
def train_ibot(args):
    ...
    if args.mode == "predefined-clips":
        dataset = VidORiBOTFrameDatasetODIS(
        ...
        from loader_vidor_odis_probabilistic import TemporalWindowVidORDataset

        dataset = TemporalWindowVidORDataset(
            frames_root=args.data_path,  # e.g. "dataset/extracted_frames"
            ann_root=args.boxes_root,  # e.g. "dataset/training"
            split="train",
            transform=transform,
            patch_size=pred_size,
            pred_ratio=args.pred_ratio,
            ...
            patch_obj_assignment_strategy=args.patch_obj_assignment_strategy,
            instance_level=args.instance_level,
            neighbour_time_window=args.neighbour_time_window,
            neighbour_tries=args.neighbour_tries,
        )
...
class iBOTLoss(nn.Module):
    ...
    def forward(
        ...
        if self.lambda_timeobj > 0:
            ...
            elif self.obj_time_matching == "tg2sg_all2all":
                for q in range(len(teacher_obj_c)):
                    for v in range(len(student_neighbor_obj_c)):
                        obj_assignments_mask = obj_neighbor_labels[v].clamp(
                            max=1.0
