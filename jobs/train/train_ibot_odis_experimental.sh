...
#SBATCH --output=slurm_outputs/train_ibot_vidor_obj/ibot_%A.out
#SBATCH --error=slurm_outputs/train_ibot_vidor_obj/ibot_%A.err
#SBATCH --array=0
###############################################################################
# 1. Environment
###############################################################################
...
###############################################################################
VIDOR_ZIP=/scratch/project_462000938/odis/vidor_36Kclips_25s.zip
...
NAME=vidor_odis_${loss_suffix_i}_25sec_bs32_withsharedMIMMask\
...
srun --cpu-bind=mask_cpu=${CPU_MASKS} \
     ...
     "${CONTAINER}" \
     "${CODE_DIR}/run.sh" \
       python -u "${CODE_DIR}/VIDOR_ODIS/ibot_vidor_obj_wmasks.py" \
            --data_path "${TMP_VIDOR}" \
