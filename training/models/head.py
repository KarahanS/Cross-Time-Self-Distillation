...
class iBOTHead(DINOHead):
    ...
    def __init__(self, *args, patch_out_dim=8192, num_object_tokens=1, norm=None, act='gelu', last_norm=None, 
        ...
        if not shared_head:
            if bottleneck_dim > 0:
                self.last_layer_patch = nn.utils.weight_norm(nn.Linear(bottleneck_dim, patch_out_dim, bias=False))
                self.last_layer_patch.weight_g.data.fill_(1)
                if norm_last_layer:
                    self.last_layer_patch.weight_g.requires_grad = False
                self.last_layer_obj = nn.utils.weight_norm(nn.Linear(bottleneck_dim, patch_out_dim, bias=False))
                self.last_layer_obj.weight_g.data.fill_(1)
                if norm_last_layer:
                    self.last_layer_obj.weight_g.requires_grad = False
            else:
                self.mlp_patch = nn.Linear(hidden_dim, patch_out_dim)
                self.mlp_obj = nn.Linear(hidden_dim, patch_out_dim)
                self.last_layer_patch = None

            self.last_norm_obj = self._build_norm(last_norm, patch_out_dim, affine=False, **kwargs)
            self.last_norm_patch = self._build_norm(last_norm, patch_out_dim, affine=False, **kwargs)
        else:
            if bottleneck_dim > 0:
                self.last_layer_patch = self.last_layer
                self.last_layer_obj = self.last_layer
            else:
                self.mlp_patch = self.mlp[-1]
                self.mlp_obj = self.mlp[-1]
                self.last_layer_patch = None

            self.last_norm_obj = self.last_norm
            self.last_norm_patch = self.last_norm
    ...
    def forward(self, x):
        ...
        if self.last_layer is not None:
            ...
            x2 = self.last_layer_obj(x[:, 1:self.num_object_tokens+1])
            x3 = self.last_layer_patch(x[:, self.num_object_tokens+1:])
        else:
            x = self.mlp[:-1](x)
            x1 = self.mlp[-1](x[:, 0])
            x2 = self.mlp_obj(x[:, 1:self.num_object_tokens+1])
            x3 = self.mlp_patch(x[:, self.num_object_tokens+1:])
        
        if self.last_norm is not None:
            x1 = self.last_norm(x1)
            x2 = self.last_norm_obj(x2)
            x3 = self.last_norm_patch(x3)
