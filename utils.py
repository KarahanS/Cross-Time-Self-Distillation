...
def restart_from_checkpoint(ckp_path, run_variables=None, **kwargs):
    ...
    """
    print("Restarting from checkpoint: {}".format(ckp_path))
    ...
    def forward(
        ...
        **kwargs,
        ...
        if head_only:
            ...
            # x is a Tensor [N, D_backbone] – no cropping logic needed
            return self.head(x)
        ...
        for end_idx in idx_crops:
            ...
            assert len(_out) == 2, "Backbone should return 2 outputs"

            cls_tok, all_tok = _out

            cls_list.append(cls_tok)
            all_list.append(all_tok)

            if start_idx == 0:
                output = all_tok
            else:
                output = torch.cat((output, all_tok))
            start_idx = end_idx
        # concatenate
        cls_matrix = torch.cat(cls_list)  # (B, embed)
        all_tokens = torch.cat(all_list)  # (B, L, embed)

        head_in = all_tokens if all_tokens is not None else cls_matrix
        # Run the head forward on the concatenated features.
        output_ = self.head(head_in)
        # output_ = (output for CLS token, output for object tokens, output for patch tokens)
        # (x_1, x_2, x_3) = output_
        # x1 is [B, out_dim] for the first token (e.g., CLS token)
        # x2 is [B, num_object_tokens, out_dim] for object tokens
        # x3 is [B, H*W, out_dim] for patch tokens

        # if return_backbone_feat, we return the token embeddings (before mapping them to output dimension using head)
        # output = (B, 1 + O + H*W, embed) so L = 1 + O + H*W
