...
import utils
import models_ibot_wattn as models  # ibot models

from torch import nn
from torchvision import transforms as pth_transforms
from loader_imagenet_mine import ImagenetBBibot
...
def extract_feature_pipeline(args):
    ...
    dataset_train = ImagenetBBibot(traindir, boxes_root=args.boxes_root, transform=transform, bb_transform=bb_transform, train=True)
    dataset_val = ImagenetBBibot(valdir, boxes_root=args.boxes_root, transform=transform, bb_transform=bb_transform, train=False)
...
def extract_features(model, data_loader, n, avgpool, use_cuda=True, multiscale=False):
    ...
    features = None
    labels = None
    for samples, obj_attn_masks, labs, index in metric_logger.log_every(data_loader, 10):
        ...
        def forward_single(samples):
            intermediate_output = model.get_intermediate_layers(samples, n, cls_attn_mask=obj_attn_masks)
            if avgpool == 0:
                # norm(x[:, 0])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--boxes_root', default='/path/to/imagenet/bboxes', type=str,
