...
###############################################################################
CKPT_DIR=/scratch/project_462000938/checkpoints/ibot_doubling_experiment
...
srun --cpu-bind=mask_cpu=${CPU_MASKS} \
     ...
     "${CONTAINER}" \
     "${CODE_DIR}/run.sh" \
       python -u "${CODE_DIR}/ibot.py" \
            --dataset VidORImageMask \
            --data_path "${TMP_VIDOR}" \
            ...
            --wandb_project vidor \
            --wandb_entity agape \
            --wandb_run_name ibot_doubling_experiment
