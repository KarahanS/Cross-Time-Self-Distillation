...
from loader_imagenet import ImagenetBBV2LP
import utils
import models
...
def train(model, linear_classifier, optimizer, loader, epoch, n, avgpool):
    ...
    for inp, target, obj_attn_mask in metric_logger.log_every(loader, 20, header):
        ...
        with torch.no_grad():
            ...
            if avgpool == 0:
                # norm(x[:, 0])
                output = [x[:, 1] for x in intermediate_output]
            elif avgpool == 1:
                # x[:, 1:].mean(1)
                output = [torch.mean(intermediate_output[-1][:, 2:], dim=1)]
            elif avgpool == 2:
                # norm(x[:, 0]) + x[:, 1:].mean(1)
                output = [x[:, 1] for x in intermediate_output] + [
                    torch.mean(intermediate_output[-1][:, 2:], dim=1)
                ]
            else:
